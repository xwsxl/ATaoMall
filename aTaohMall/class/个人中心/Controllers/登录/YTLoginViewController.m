...
@interface YTLoginViewController ()<UITextFieldDelegate,NameAndPassWordDelegate>
{
...
- (void)viewDidLoad {
    ...
    self.YTTF.hidden=YES;
    
    //显示验证码界面
    self.authcodeView = [[AuthcodeView alloc] initWithFrame:CGRectMake(self.YTTF.frame.size.width-120, 7, 70, 30)];
    
    [self.YTTF addSubview:self.authcodeView];
    
    
    UIButton *button=[UIButton buttonWithType:UIButtonTypeCustom];
    
    button.frame=CGRectMake(250, 7, 70, 30);
    
    [self.YTTF addSubview:button];
    ...
    self.YTLoginButton.enabled=NO;
    
    [self.YTLoginButton setBackgroundImage:[UIImage imageNamed:@"为选中按钮"] forState:0];
    
    self.view.frame=[UIScreen mainScreen].bounds;
    
    
    //设置代理
    self.YTUserNameTF.delegate=self;
    self.YTPassWordTF.delegate=self;
...
//    }
    ...
    if ([self.backString isEqualToString:@"100"]) {
        ...
        self.YTUserNameTF.text=self.userName;
        self.YTPassWordTF.text=self.userPassWord;
        
    }else{
        
        self.YTUserNameTF.text=[userDefaultes stringForKey:@"myName"];
        self.YTPassWordTF.text=nil;
    }
    //密码
    [ZZLimitInputManager limitInputView:self.YTUserNameTF maxLength:20];
    //用户名
    [ZZLimitInputManager limitInputView:self.YTPassWordTF maxLength:20];
    
    [ZZLimitInputManager limitInputView:self.YTMessageTF maxLength:4];
    
}
-(void)setUserName:(NSString *)name andPassWord:(NSString *)password
{
    self.YTUserNameTF.text=name;
    
    self.YTPassWordTF.text=password;
}
-(void)YanBtnClick
{
...
- (IBAction)KeShiBtnClick:(UIButton *)sender {
    ...
    if (sender.selected) {
        ...
        self.YTPassWordTF.secureTextEntry = YES;
        
        self.YTImageView.image=[UIImage imageNamed:@"不可视"];
        
    }else{
        
        self.YTPassWordTF.secureTextEntry = NO;
            ...
            if (!isMatch1) {
                ...
                if (self.YTPassWordTF.text.length == 0) {
                    ...
                    if (self.YTPassWordTF.text.length < 6){
                        ...
                        if([self.YTPassWordTF.text rangeOfString:@" "].location != NSNotFound){
...
//                                self.YTUserNameTF.text=@"";
//                                self.YTPassWordTF.text=@"";
                        ...
                        if([self.YTPassWordTF.text rangeOfString:@" "].location != NSNotFound){
                            ...
                            [self presentViewController: alertCon animated: YES completion: nil];
                                ...
                                if (self.YTMessageTF.text.length==0) {
                                    ...
                                    NSString *astring01 = self.YTMessageTF.text;
...
-(void)NewLogin
{
    ...
    NSDictionary *dic = @{@"phone":self.YTUserNameTF.text,@"password":self.YTPassWordTF.text};
            ...
            for (NSDictionary *dic1 in dic) {
                ...
                if ([dic1[@"status"] isEqualToString:@"10000"]) {
                    ...
                    if (_delegate && [_delegate respondsToSelector:@selector(setSigenWithString:andStatusWithString:andIntegralWithString:andPhoneWithString:andHeaderImageWithString:andUserId: andUserName:)]) {
                        [_delegate setSigenWithString:dic1[@"sigen"] andStatusWithString:dic1[@"status"] andIntegralWithString:dic1[@"integral"] andPhoneWithString:dic1[@"phone"] andHeaderImageWithString:dic1[@"portrait"] andUserId:dic1[@"userid"] andUserName:self.YTUserNameTF.text];
                    }
                    
                    //缓存用户名
                    [UserMessageManager UserName:self.YTUserNameTF.text];
                    
                    [UserMessageManager UserNewName:self.YTUserNameTF.text];
                    
                    //缓存用户密码
                    [UserMessageManager UserPassWord:self.YTPassWordTF.text];
...
- (void)viewWillDisappear:(BOOL)animated{
    ...
    [self.YTPassWordTF resignFirstResponder];
    [self.YTMessageTF resignFirstResponder];
}
...
-(void)textFieldDidBeginEditing:(UITextField *)textField
{
    ...
    if (textField==self.YTUserNameTF) {
        
        NSLog(@"111");
        
        if (self.YTPassWordTF.text.length>0) {
            
            self.YTLoginButton.enabled=YES;
            [self.YTLoginButton setBackgroundImage:[UIImage imageNamed:@"按钮"] forState:0];
    ...
    }else if (textField==self.YTPassWordTF){
        NSLog(@"222");
        if (self.YTUserNameTF.text.length>0) {
