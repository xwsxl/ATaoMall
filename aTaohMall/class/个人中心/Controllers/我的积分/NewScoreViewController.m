...
@interface NewScoreViewController ()<UITableViewDelegate,UITableViewDataSource,DJRefreshDelegate,FooterViewDelegate,NewScoreHeaderViewDelegate>
{
...
-(void)setTop
{
    ...
    [button1 addTarget:self action:@selector(AllScoreButClick:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [button2 addTarget:self action:@selector(ShouRuScoreButClick:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)getData
{
    ...
    NSString *url = [NSString stringWithFormat:@"%@getUserIntegralAllList_mob.shtml",URL_Str];
