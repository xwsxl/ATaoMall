...
#define GGDefaultFont [UIFont boldSystemFontOfSize:14]
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        if (CGSizeEqualToSize(frame.size, CGSizeZero)) {
            frame.size = CGSizeMake(100, 25);
            self.frame = frame;
        }
        self.timeBackgroundColor = [UIColor clearColor];//GGDefaultTimeBackgroundColor;
        self.timeTextColor       = [UIColor whiteColor];//GGDefaultTimeTextColor;
        self.colonColor          = [UIColor whiteColor];//GGDefaultColonColor;
...
- (void)layoutSubviews {
    ...
    CGFloat heigth      = 20;//self.frame.size.height;
    CGFloat colonLabelW = 5;//7.5;
    CGFloat timeLabelW  = 20;//(self.frame.size.width - 2 * colonLabelW) / 3.0;
    
    CGFloat hourLabelX       = 10;
    ...
    self.hourLabel.frame       = CGRectMake(hourLabelX, 10, timeLabelW+10, heigth);
    self.minuteLabel.frame     = CGRectMake(minuteLabelX+10, 10, timeLabelW, heigth);
    self.secondLabel.frame     = CGRectMake(secondLabelX+10, 10, timeLabelW, heigth);
    
    self.leftColonLabel.frame  = CGRectMake(leftColonLabelX+10, 10, colonLabelW, heigth);
    self.rigthColonLabel.frame = CGRectMake(rigthColonLabelX+10, 10, colonLabelW, heigth);
...
- (void)setTimeTextColor:(UIColor *)timeTextColor {
    _timeTextColor = timeTextColor ? timeTextColor : [UIColor whiteColor];//GGDefaultTimeTextColor;
...
- (void)setColonColor:(UIColor *)colonColor {
    _colonColor = colonColor ? colonColor : [UIColor whiteColor];//GGDefaultColonColor;
