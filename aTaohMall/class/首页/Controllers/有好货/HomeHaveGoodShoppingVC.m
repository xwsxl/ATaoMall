...
@interface HomeHaveGoodShoppingVC ()<XLNaviViewDelegate,UITableViewDelegate,UITableViewDataSource>
{
...
@implementation HomeHaveGoodShoppingVC
...
-(void)getData
{
    ...
    str=[self.dataSource componentsJoinedByString:@"_"];
    }
    NSDictionary *params=@{@"ids":str};
    WKProgressHUD *hud=[WKProgressHUD showInView:self.view withText:@"" animated:YES];

    [ATHRequestManager requestforgetGoodGoodsListWithParams:params successBlock:^(NSDictionary *responseObj) {
        ...
        if ([responseObj[@"status"] isEqualToString:@"10000"]) {
            ...
            for (NSDictionary *dic in responseObj[@"list"]) {
              /*  "id": 14,
                ...
                "gid": 385,
                "mid": "2242",
                "scopeimg": "http://image.anzimall.com/union/upload/7d49080b0000170a.jpg",
                "pay_maney": "72.80",
                "pay_integer": "18.20",
                "amount": "3",
                "is_attribute": "1",
...
-(void)initNavi
{
    XLNaviView *navi=[[XLNaviView alloc] initWithMessage:self.naviTitle ImageName:@""];
    navi.delegate=self;
