...
@interface CartChoseView()<LoginMessageDelegate,UIAlertViewDelegate>
{
...
@implementation CartChoseView

@synthesize alphaiView,whiteView,img,lb_detail,lb_price,lb_stock,mainscrollview,sizeView,colorView,countView,bt_sure,bt_cancle,lb_line,colorarr,sizearr,stock,stockdic,arr3,arr4,arr5,arr3View,arr4View,lb_arr3,lb_arr4,Goods_type,Goods_status,gid1;
...
-(void)initTypeView:(NSArray *)sizeArr :(NSArray *)colorArr :(NSArray *)Arr3 :(NSArray *)Arr4 :(NSArray *)Arr5 :(NSDictionary *)stockDic andGoodsImageView:(NSString *)imgStr andMoney:(NSString *)money andJiFen:(NSString *)jifen andKuCun:(NSString *)kucun andCount:(NSString *)count andGoods_type:(NSString *)type andStatus:(NSString *)status andGid:(NSString *)gid andYTBack:(NSString *)ytback
{
    ...
    for (NSInteger i = 0; i<[arr5 count]; i++) {
        ...
        if (i==0) {
            ...
            //尺码
            sizeView = [[TypeView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 50) andDatasource:sizearr :str];
        ...
        if (i==1) {
            //颜色分类
            colorView = [[TypeView alloc] initWithFrame:CGRectMake(0, fheight, self.frame.size.width, 50) andDatasource:colorarr :str];
        ...
        if (i == 2) {
            arr3View = [[TypeView alloc] initWithFrame:CGRectMake(0,fheight+arr3View.frame.size.height, self.frame.size.width, 50) andDatasource:arr3 :str];
            arr3View.delegate = self;
            [mainscrollview addSubview:arr3View];
            arr3View.frame = CGRectMake(0,fheight, self.frame.size.width, arr3View.height);
            fheight += arr3View.frame.size.height;
        }
        if (i == 3) {
            arr4View = [[TypeView alloc] initWithFrame:CGRectMake(0, fheight + arr4View.frame.size.height, self.frame.size.width, 50) andDatasource:arr4 :str];
            arr4View.delegate = self;
            [mainscrollview addSubview:arr4View];
            arr4View.frame = CGRectMake(0,fheight, self.frame.size.width, arr4View.height);
    ...
    countView.frame = CGRectMake(0, fheight, self.frame.size.width, 50);
...
-(void)getDatas
{
    ...
    NSDictionary *dic = @{@"smallIds":self.ShuXingString,@"userId":self.userId,@"gid":gid1};
