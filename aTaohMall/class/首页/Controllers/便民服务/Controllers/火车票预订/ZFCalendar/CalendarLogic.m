...
- (NSMutableArray *)reloadCalendarView:(NSDate *)date  selectDate:(NSDate *)selectdate needDays:(int)days_number ChuanRu:(NSString *)chuanru Back:(NSString *)back
{
    ...
    for (int i = 0; i <= months; i++) {
        ...
        [self calculateDaysInCurrentMonthWithDate:month andArray:calendarDays ChuanRu:chuanru Back:back];
...
- (void)calculateDaysInCurrentMonthWithDate:(NSDate *)date andArray:(NSMutableArray *)array ChuanRu:(NSString *)chuanru Back:(NSString *)back
{
...
//        calendarDay.Chinese_calendar = [self LunarForSolarYear:components.year Month:components.month Day:i];
        ...
        [self changStyle:calendarDay ChuanRu:chuanru Back:back];
        [array addObject:calendarDay];
    }
}
...
- (void)changStyle:(CalendarDayModel *)calendarDay ChuanRu:(NSString *)chanru Back:(NSString *)back;
{

    NSLog(@"backbackbackback================%@",back);
    
    NSDateComponents *calendarToDay;
    NSDateComponents *calendarbefore;
    NSDateComponents *calendarSelect;
    
    NSDateComponents *calendargodate;
    
    
        
        calendarToDay  = [today YMDComponents];//今天
        calendarbefore = [before YMDComponents];//最后一天
        calendarSelect = [select YMDComponents];//默认选择的那一天
    calendargodate=[[[NSDateFormatter shareDateFormatter] dateFromString: chanru ] YMDComponents];
...
//            
//            NSLog(@"==111=%lu===%lu==%lu",(unsigned long)calendarDay.year,(unsigned long)calendarDay.month,(unsigned long)calendarDay.day);
//            
...
//        }
     //   昨天乃至过去的时间设置一个灰度
        if ((calendargodate.year>calendarDay.year)||(calendargodate.year>=calendarDay.year&&calendargodate.month>calendarDay.month)||(calendargodate.year >= calendarDay.year &
            calendargodate.month >= calendarDay.month &
            calendargodate.day > calendarDay.day)){
            ...
            NSLog(@"==222=%lu===%lu==%lu",(unsigned long)calendarDay.year,(unsigned long)calendarDay.month,(unsigned long)calendarDay.day);
    ...
    //这里来判断节日
    //今天
    if ([back isEqualToString:@"100"]) {
        calendarToDay=[[NSDate date] YMDComponents];
    }
    if (calendarToDay.year == calendarDay.year &&
        calendarToDay.month == calendarDay.month &&
        calendarToDay.day == calendarDay.day) {
