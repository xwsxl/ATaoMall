...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.headerReferenceSize = CGSizeMake([UIScreen mainScreen].bounds.size.width, 70.0f);//头部视图的框架大小
        self.footerReferenceSize = CGSizeMake([UIScreen mainScreen].bounds.size.width, 20.0f);//头部视图的框架大小
        
        self.itemSize = CGSizeMake([UIScreen mainScreen].bounds.size.width/7, 30.0f);//每个cell的大小
