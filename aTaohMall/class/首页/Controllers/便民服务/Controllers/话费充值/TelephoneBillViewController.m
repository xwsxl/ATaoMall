...
@interface TelephoneBillViewController ()<UITextFieldDelegate,ABPeoplePickerNavigationControllerDelegate, UINavigationControllerDelegate,UITabBarControllerDelegate,UITabBarDelegate,CNContactPickerDelegate,LoginMessageDelegate>
{
...
//    self.navigationController.navigationBar.hidden=NO;
    ...
    UIView *view1 = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 10)];
    view1.backgroundColor = [UIColor colorWithRed:244/255.0 green:244/255.0 blue:244/255.0 alpha:1.0];
...
-(void)initPhoneView
{
    ...
    PayView = [[UIView alloc] initWithFrame:CGRectMake(15, [UIScreen mainScreen].bounds.size.height-49-38-65-40+10-KSafeAreaBottomHeight, [UIScreen mainScreen].bounds.size.width-30, 38)];
...
-(void)initNumberView
{
    ...
    NSArray *MianZhi = @[@"10元",@"20元",@"30元",@"50元",@"100元",@"300元"];
    ...
    for (int i = 0; i < 6; i++) {
        ...
        abBtn.adjustsImageWhenHighlighted=NO;
...
-(void)chargePhone:(ABButton*)sender{
    ...
    NSArray *MianZhi = @[@"10",@"20",@"30",@"50",@"100",@"300"];
...
-(BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    NSLog(@"==111==%@=====%@===range==%lu",textField.text,string,(unsigned long)range.location);
    
    NSLog(@"==222==%@",[NSString stringWithFormat:@"%@%@",textField.text,string]);
    ...
    if ((unsigned long)range.location == 0) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            button.layer.borderColor=[RGB(204, 204, 204) CGColor];
            
            [button setTitleColor:[UIColor colorWithRed:204/255.0 green:204/255.0 blue:204/255.0 alpha:1.0] forState:0];
            [button setBackgroundImage:[UIImage imageNamed:@"框未选425"] forState:0];
            [button removeTarget:self action:@selector(chargePhone:) forControlEvents:UIControlEventTouchUpInside];
            [button addTarget:self action:@selector(plzInputPhoneNum) forControlEvents:UIControlEventTouchUpInside];
        }
...
-(void)getDatas
{
    ...
    NSDictionary *dic = @{@"type":@"0",@"phone":bankNo};
        ...
        if (codeKey && content) {
            ...
            if ([dic[@"status"] isEqualToString:@"10000"]) {
                ...
                for (int i = 0; i < 6; i++) {
                    ...
                    [button removeTarget:self action:@selector(plzInputPhoneNum) forControlEvents:UIControlEventTouchUpInside];
                    [button addTarget:self action:@selector(chargePhone:) forControlEvents:UIControlEventTouchUpInside];
                    if (i == 0) {
...
-(void)submitMoneyPay
{
    ...
    NSDictionary *dic = @{@"sigen":self.sigen,@"type":@"0",@"phone":bankNo,@"price":self.Price,@"flow_size":@"",@"flow_id":@"",@"clientId":@"3"};
