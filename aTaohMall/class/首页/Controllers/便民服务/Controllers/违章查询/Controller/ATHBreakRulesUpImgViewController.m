//
//  ATHBreakRulesUpImgViewController.m
//  违章查询
//
...
@interface ATHBreakRulesUpImgViewController ()<UIActionSheetDelegate,UIImagePickerControllerDelegate>
{
...
@implementation ATHBreakRulesUpImgViewController
/*******************************************************      控制器生命周期       ******************************************************/
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)initUpImgView
{
    ...
    label.text = @"提交的违章需要上传行驶证正副本照片，需并排放在一张A4纸上面,不能有反光,必须清晰,图片为3M以内的jpg或者png格式。";
...
- (void)upLoadImage{
    ...
    ASIFormDataRequest * _requestUpLoad= [ASIFormDataRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@illegalUploadPictures_mob.shtml",URL_Str]]];
    ...
    [_requestUpLoad addPostValue:self.orderModel.orderNo forKey:@"orderno"];
    [_requestUpLoad addPostValue:self.papersType forKey:@"papersType"];
