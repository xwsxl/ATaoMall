...
#import "AirPlaneBackViewController.h"
...
@interface AirPlaneBackViewController ()<UITableViewDataSource,UITableViewDelegate,DJRefreshDelegate,AirSelectDelegate,AirPlaneDetailBackDetegate>
{
...
@implementation AirPlaneBackViewController
...
-(void)GetDateDatas
{
    ...
    NSDictionary *dict = @{@"time":self.time,@"type":@"1"};
...
-(void)initTopView
{
    ...
    //    UIView *write = (UIView *)[self.view viewWithTag:999];
    ...
    if (self.Index-1 > 4) {
        ...
        CGPoint position = CGPointMake(80*(self.Index-3), 0);
...
-(void)RiLiBtnClick
{
    ...
    NSString *goDate=[[NSDateFormatter shareDateFormatter] stringFromDate:[NSDate date]];
    if ([kUserDefaults objectForKey:@"AirDate"]!=nil) {
        goDate=[kUserDefaults objectForKey:@"AirDate"];
    }
    
    [vc setAirPlaneToDay:365 ToDateforString:goDate andString:self.time back:@"100"];//飞机初始化方法
...
-(void)initTableView
{
    ...
    _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, KSafeAreaTopNaviHeight+45, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height-KSafeAreaTopNaviHeight-KSafeAreaBottomHeight-50-45) style:UITableViewStylePlain];
...
//        cell.StopOverLab.hidden=NO;
//    }else
//    {
//        cell.StopOverLab.hidden=YES;
//    }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    AirPlaneBackDetailViewController *vc = [[AirPlaneBackDetailViewController alloc] init];
...
//    [RecordAirManger removeAirline_airway1];
