...
@interface YTScrolling ()<UICollectionViewDataSource,UICollectionViewDelegate>
{
    /**下载图片*/
...
{
    if (self.delegate &&  [self.delegate respondsToSelector:@selector(ytScrolling:clickAtIndex:)]) {
        [self.delegate ytScrolling:self clickAtIndex:indexPath.row %self.photos.count];
    }
}
