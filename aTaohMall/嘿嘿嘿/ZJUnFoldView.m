                               ...
                               location:(UnFoldButtonLocation)location
{
    ...
    if (_isMutilLine) { // 4.1 有多行
        ...
        if (_isSurpassFoldLine)
        {
            ...
            _foldButtonX=maxWidth - _unFoldButtonW;
            ...
            if (_isFoldLinesMoreOne) {
                tempContentAttributedString = [ZJUnFoldView getLineNumBeforeAttrStingFromAttrStr:contentAttributedString lineNum:foldLines deleteStringLength:unFoldAttrStr.unFoldAttributedString.length+1 maxWidth:maxWidth];
                NSRange range=NSMakeRange(0,[tempContentAttributedString length]);
                
                
                if (unFoldAttrStr.paragraphStyle) {
                    [tempContentAttributedString addAttribute:NSParagraphStyleAttributeName value:unFoldAttrStr.paragraphStyle range:range];
                }
...
- (void)unFoldAction
{
    ...
    if (_delegate) {
        [_delegate setIsUnfoldWithTag:self.tag];
    }
